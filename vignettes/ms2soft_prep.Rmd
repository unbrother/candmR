...
The base folder structure contains the inputs folder, which has both input files explained before; the outputs folder, which is used at the moment to contain the output database produced by the web scraping process. The outputs folder is designed so that users can save different databases produced within R, results and insights, the stations locations geographic file and graphs as needed. The R folder contains the main code and some deprecated scripts, the main code is inside the get_data.R file, while the functions.R has the functions used within the main code and the theme.R is used to create graphs (it is not fully developed at the moment).
To start a new request, users must open the Webscraping R project, this ensures that all files and folders live within the same working directory, and then open the get_data.R file. Nothing else is needed and editing other files is recommended only for advanced users.
